async function GetVentaIDSoap(id) {
  try {
    const xml = `<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
    <Body>
        <obtenerVentaPorID xmlns="http://service/">
            <id xmlns="">${id}</id>
        </obtenerVentaPorID>
    </Body>
</Envelope>`;
    ...
    const getVentaID = Array.from(xmlDoc.getElementsByTagName("return")).map(
      (node) => {
